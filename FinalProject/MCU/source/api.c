      ...
      //      sscanf(Oper1fFromFlash, "%2x", &Oper2ToInt);
        ...
        case '3':
         ...
         //   sscanf(Oper1fFromFlash, "%2x", &Oper2ToInt);
            ...
            while (Oper2ToInt>0)
            {
                rra_lcd(Oper2ToInt);         //rotate right
                Oper2ToInt = 0;
            }
            break;
        case '4':
            ...
            Operand1Flash[1] = *Flash_ptrscript++;
            y = y + 2;
       //     sscanf(Oper1fFromFlash, "%2x", &delay_time); //    delay
...
//                rra_lcd
//-------------------------------------------------------------
void rra_lcd(int delay) {
    for (int i = 0; i < 32; i++) {
        LCD_rotate_right();           // Rotate the LCD display by one character
        delay_ms(delay * 10);         // Delay, converting units of 10ms to milliseconds
    }
}
*/
...
//-------------------------------------------------------------
//               NEW 3. Rotate right LCD
//-------------------------------------------------------------

void rra_lcd(int value){
        lcd_clear();
        lcd_home();
        int2str(HelperStr , value);
        for(i=0;i<16; i++){
           lcd_puts(HelperStr);
           lcd_cursor_right();
           timer_call_counter();
