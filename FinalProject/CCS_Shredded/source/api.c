...
void StepperUsingJoyStick(){
    ...
    while (counter != 0 && state==state0 && stateStepp==stateJSRotate){ //counter != 0 We did Calibrate first
        ...
        if (!( Vr[1] > 400 && Vr[1] < 600 && Vr[0] > 400 && Vr[0] < 600)){ //Assuming they  still in center the pressure weak
            Vrx = Vr[1] -512;  //Calculation to maintain a ratio that the center will be zero
            Vry = Vr[0] -512;  //Calculation to maintain a ratio that the center will be zero
...
void ExecuteScript(void)
{
    ...
    for (K = 0; K < 20;K++){
        ...
        case '1':
            ...
            inc_lcd(Oper2ToInt);    //Count up from zero to x with delay d
            break;
        ...
        case '2':
            ...
            Oper2ToInt = hex2int(Operand1Flash);
            dec_lcd(Oper2ToInt);    //Count down from x with delay d
            break;
        ...
        case '3':
            ...
            Oper2ToInt = hex2int(Operand1Flash);
            rra_lcd(Oper2ToInt);        //Rotate right onto LCD
            break;
